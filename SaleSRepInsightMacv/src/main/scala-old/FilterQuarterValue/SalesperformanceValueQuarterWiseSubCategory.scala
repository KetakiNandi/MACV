...
object SalesperformanceValueQuarterWiseSubCategory {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse (""), Try(fields(17)) getOrElse (""), Try(fields(13)) getOrElse (""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.SalesperformanceValueQuarterWiseSubCategory")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.SalesperformanceValueQuarterWiseSubCategory")
    ...
    val placeTotalFiltercountQuarter=TotalFiltercount.map(s=>(s._2,s._1,s._3))//City,month,Quantity
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("SubCategory",rdd._1).append("Quarter",rdd._2).append("Amount",rdd._3))
