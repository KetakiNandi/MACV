...
object DateWisePosTypeWiseWeekDaySalesQuarter {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWisePosTypeWiseWeekDaySalesQuarter")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWisePosTypeWiseWeekDaySalesQuarter")
    ...
    val finalResult = convertMonth.map(s=>(s._1+"#"+s._2+"#"+s._4,s._3.toInt)).reduceByKey(_+_).map(s=>(s._1.split("#")(1),s._1.split("#")(0),s._2,s._1.split("#")(2)))
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._4).getTime)).append("Quarter",rdd._2).append("PosType",rdd._1).append("Quantity",rdd._3))
