...
object DateWiseCategoryWiseWeekDaySalesQuarter {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseCategoryWiseWeekDaySalesQuarter")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseCategoryWiseWeekDaySalesQuarter")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp", today).append("date",new Date(df.parse(rdd._4).getTime)).append("Quarter", rdd._1).append("Category", rdd._2).append("Quantity", rdd._3))
