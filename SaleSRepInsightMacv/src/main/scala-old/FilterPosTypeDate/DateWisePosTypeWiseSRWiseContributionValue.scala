...
object DateWisePosTypeWiseSRWiseContributionValue {
  ...
  = {
    ...
    (Try(fields(5)) getOrElse(""),Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),
      Try(fields(18)) getOrElse(""),Try(fields(29)) getOrElse(""),Try(fields(20)) getOrElse(""),Try(fields(1)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWisePosTypeWiseSRWiseContributionValue")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWisePosTypeWiseSRWiseContributionValue")
    ...
    // SalefinalResult.foreach(println)
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp", today).append("date",new Date(df.parse(rdd._6).getTime)).append("PosType", rdd._1).append("City", rdd._5).append("SourceSite", rdd._2).append("SalespersonId", rdd._3).append("Salesperson", rdd._4).append("Amount", rdd._7))
