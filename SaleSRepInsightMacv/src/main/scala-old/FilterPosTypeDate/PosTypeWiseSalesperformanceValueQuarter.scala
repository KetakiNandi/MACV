...
object PosTypeWiseSalesperformanceValueQuarter {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(18)) getOrElse(""),Try(fields(29)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.PosTypeWiseSalesperformanceValueQuarter")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.PosTypeWiseSalesperformanceValueQuarter")
    ...
    val placeTotalFiltercountQuarter=TotalFiltercount.map(s=>(s._2,s._1,s._3))
    ...
    val results = ChecksubstringInQuarter.collect()
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("PosType",rdd._1).append("Quarter",rdd._2).append("Amount",rdd._3))
