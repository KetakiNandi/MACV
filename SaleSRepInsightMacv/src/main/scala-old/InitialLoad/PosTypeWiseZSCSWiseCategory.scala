...
object PosTypeWiseZSCSWiseCategory {
  ...
  = {
    ...
    (Try(fields(0)) getOrElse(""),Try(fields(22)) getOrElse(""),Try(fields(21)) getOrElse(""),Try(fields(20)) getOrElse(""),Try(fields(5)) getOrElse(""),Try(fields(12)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.PosTypeWiseZSCSWiseCategory")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.PosTypeWiseZSCSWiseCategory")
    ...
    val DistinctZoneAndPosCode = TotalFilter.map(s=>(s._1,s._2,s._3,s._4,s._5,s._6)).distinct().map(s=>(s._1,s._2+"@"+s._3+"@"+s._4+"@"+s._5+"@"+s._6))

     //DistinctZoneAndPosCode.foreach(println)

    // println("ssssssssssssssssss",DistinctZoneAndPosCode.count())

    val JoinTwoCollections = DistinctZoneAndPosCode.join(disPosMaster).map(s=>(s._2._1,s._2._2)).map(s=>(s._1.split("@")(0),s._1.split("@")(1),s._1.split("@")(2),s._1.split("@")(3),s._1.split("@")(4),s._2))
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("Zone",rdd._1).append("State",rdd._2).append("City",rdd._3).append("SourceSite",rdd._4).append("Category",rdd._5).append("PosType",rdd._6))
