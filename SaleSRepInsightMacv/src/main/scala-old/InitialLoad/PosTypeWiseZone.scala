...
import FilterCategory.CategoryWisePoSTargetAchievement.extractData
...
object PosTypeWiseZone {
  ...
  = {
    ...
    (Try(fields(0)) getOrElse(""),Try(fields(22)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.PosTypeWiseZone")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.PosTypeWiseZone")
   ...
   // disPosMaster.foreach(println)
    ...
    val DistinctZoneAndPosCode = TotalFilter.map(s=>(s._1,s._2)).distinct()

    //TotalFilter.foreach(println)

    //println("ssssssssssssssssss",disPosMaster.count())

    val JoinTwoCollections = DistinctZoneAndPosCode.join(disPosMaster).map(s=>(s._2._1,s._2._2))
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("Zone",rdd._1).append("PosType",rdd._2))
