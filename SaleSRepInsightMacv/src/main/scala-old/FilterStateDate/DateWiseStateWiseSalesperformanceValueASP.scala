...
object DateWiseStateWiseSalesperformanceValueASP {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(18)) getOrElse(""),Try(fields(17)) getOrElse(""),Try(fields(21)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseStateWiseSalesperformanceValueASP")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseStateWiseSalesperformanceValueASP")
...
//    val dd =JoinTwoCollections.map(s=>(s._1,s._2,s._3,s._4,s._1.substring(3,5).toInt)).filter(s=>(s._5==1))
//
//    //dd.foreach(println)
//    val ff =dd.map(s=>(s._5,s._3.toInt)).reduceByKey(_+_)
//
//    ff.foreach(println)//8029//11201825//11201825
   ...
   val finalResult = JoinTwoCollections.collect()
    ...
    finalResult.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("State",rdd._2).append("Amount",rdd._3).append("CountPerday",rdd._4))
