...
object DateWiseEmpIdWiseQuarterQuantity {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseEmpIdWiseQuarterQuantity")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseEmpIdWiseQuarterQuantity")
   ...
   // TotalFiltercount.foreach(println)
    ...
    val placeTotalFiltercountQuarter=TotalFiltercount.map(s=>(s._2,s._3,s._1,s._4,s._5))
  ...
  val finalResult=ChecksubstringInQuarter.map(s=>(s._1,s._2,s._3,s._4,s._5))
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._4).getTime)).append("SalesRepNameid",rdd._1).append("SalesRepName",rdd._2).append("Quarter",rdd._3).append("Quantity",rdd._5))
