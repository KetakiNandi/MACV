...
object DateZoneEmpIdWiseQuarterQuantity {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse (""), Try(fields(17)) getOrElse (""), Try(fields(8)) getOrElse (""),Try(fields(9)) getOrElse (""),Try(fields(22)) getOrElse (""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneEmpIdWiseQuarterQuantity")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneEmpIdWiseQuarterQuantity")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._4).getTime)).append("Zone",rdd._5).append("SalesRepNameid",rdd._1).append("SalesRepName",rdd._2).append("Quarter",rdd._3).append("Quantity",rdd._6))
