...
object DateCityEmpIdWiseCustomerDetailsFilled {
  ...
  = {
    ...
    (Try(fields(8)) getOrElse (""),Try(fields(9)) getOrElse (""),Try(fields(7)) getOrElse (""),Try(fields(1)) getOrElse ("") ,Try(fields(20)) getOrElse (""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCityEmpIdWiseCustomerDetailsFilled")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCityEmpIdWiseCustomerDetailsFilled")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("City",rdd._4).append("SalesRepNameid",rdd._2).append("SalesRepName",rdd._3).append("CustomerMobile",rdd._5).append("Count",rdd._6))
