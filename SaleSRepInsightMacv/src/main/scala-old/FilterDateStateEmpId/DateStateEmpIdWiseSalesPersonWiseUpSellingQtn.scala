...
object DateStateEmpIdWiseSalesPersonWiseUpSellingQtn {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(2)) getOrElse(""),Try(fields(5)) getOrElse(""),
      Try(fields(6)) getOrElse(""),Try(fields(14)) getOrElse("") ,Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),Try(fields(21)) getOrElse(""),Try(fields(18)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStateEmpIdWiseSalesPersonWiseUpSellingQtn")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStateEmpIdWiseSalesPersonWiseUpSellingQtn")
    ...
    splitvalue.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("UniqueItemCodeCount",rdd._2).append("SourceSite",rdd._3).append("State",rdd._6).append("SalespersonId",rdd._4).append("Salesperson",rdd._5))
