...
object DateWiseSRWiseCategoryWisePromoWiseWeekendSalesMonthQuarter {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(17)) getOrElse(""),Try(fields(8)) getOrElse("") ,Try(fields(12)) getOrElse(""),Try(fields(10)) getOrElse(""),Try(fields(11)) getOrElse(""),Try(fields(9)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseSRWiseCategoryWisePromoWiseWeekendSalesMonthQuarter")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseSRWiseCategoryWisePromoWiseWeekendSalesMonthQuarter")
    ...
    val TotalFilter = mappedSaleRepdata.filter(s=> !s._1.isEmpty && !s._2.isEmpty  && !s._3.isEmpty  && !s._4.isEmpty  && !s._5.isEmpty  && !s._6.isEmpty && !s._7.isEmpty).map(s=>(s._1+"#"+s._3+"#"+s._4+"#"+s._5.replace("-","")+"#"+s._6+"#"+s._1.substring(3,5)+"#"+s._7,s._2.toInt))
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("Quarter",rdd._6).append("SalesRepNameid",rdd._2).append("SalesRepName",rdd._8).append("Category",rdd._3).append("PromoNo",rdd._4).append("PromoName",rdd._5).append("Quantity",rdd._7))
