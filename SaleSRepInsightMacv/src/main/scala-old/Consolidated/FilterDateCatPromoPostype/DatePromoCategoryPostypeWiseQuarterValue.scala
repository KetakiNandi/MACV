...
object DatePromoCategoryPostypeWiseQuarterValue {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DatePromoCategoryPostypeWiseQuarterValue")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DatePromoCategoryPostypeWiseQuarterValue")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("Quarter",rdd._7).append("PosType",rdd._2).append("Category",rdd._3).append("PromoNo",rdd._4).append("PromoName",rdd._5).append("Amount",rdd._6))
