...
object DateSourceSiteEmpIdPostypeWiseSalesperformanceValueASP {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(18)) getOrElse(""),Try(fields(5)) getOrElse(""),Try(fields(29)) getOrElse("")
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteEmpIdPostypeWiseSalesperformanceValueASP")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteEmpIdPostypeWiseSalesperformanceValueASP")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("SourceSite",rdd._2).append("PosType",rdd._3).append("SalesRepNameid",rdd._4).append("SalesRepName",rdd._5).append("Amount",rdd._6).append("CountPerday",rdd._7))
