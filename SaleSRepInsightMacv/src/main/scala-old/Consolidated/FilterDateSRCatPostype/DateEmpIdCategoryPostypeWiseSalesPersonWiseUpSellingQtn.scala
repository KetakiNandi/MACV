...
object DateEmpIdCategoryPostypeWiseSalesPersonWiseUpSellingQtn {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(2)) getOrElse(""),Try(fields(5)) getOrElse(""),Try(fields(6)) getOrElse(""),Try(fields(14)) getOrElse("") ,Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),
      Try(fields(20)) getOrElse("") ,Try(fields(12)) getOrElse(""),Try(fields(29)) getOrElse(""),Try(fields(18)) getOrElse("") )
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateEmpIdCategoryPostypeWiseSalesPersonWiseUpSellingQtn")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateEmpIdCategoryPostypeWiseSalesPersonWiseUpSellingQtn")
    ...
    val a=TotalFilter.map(s=>(s._1,s._2,s._3,s._4,s._5,s._6,s._7,s._8,s._9,s._10.replace("-",""),s._11.toInt)).filter(s=>(s._11>0))//date,billNo,sourceSite,customername,itemCode,empId,empName,promoName,promoNo
    ...
    splitvalue.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("City",rdd._6).append("UniqueItemCodeCount",rdd._2).append("SourceSite",rdd._3).append("SalespersonId",rdd._4).append("Salesperson",rdd._5).append("Category",rdd._7).append("PosType",rdd._8))

       MongoSpark.save(sc.parallelize(newDocs))

      //println(newDocs)//Date,SR,Category,POSType
    })
  }
