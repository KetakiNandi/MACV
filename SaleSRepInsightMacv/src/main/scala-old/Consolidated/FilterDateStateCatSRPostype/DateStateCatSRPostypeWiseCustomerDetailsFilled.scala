...
object DateStateCatSRPostypeWiseCustomerDetailsFilled {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStateCatSRPostypeWiseCustomerDetailsFilled")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStateCatSRPostypeWiseCustomerDetailsFilled")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("State",rdd._2).append("Category",rdd._3).append("PosType",rdd._6).append("SalesRepNameid",rdd._4).append("SalesRepName",rdd._5).append("CustomerMobile",rdd._7).append("Count",rdd._8))
