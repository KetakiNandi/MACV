...
object DateCityCategorySRWiseQuarterQuantity {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCityCategorySRWiseQuarterQuantity")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCityCategorySRWiseQuarterQuantity")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("Quarter",rdd._7).append("City",rdd._2).append("Category",rdd._3).append("SalesRepNameid",rdd._4).append("SalesRepName",rdd._5).append("Quantity",rdd._6))
