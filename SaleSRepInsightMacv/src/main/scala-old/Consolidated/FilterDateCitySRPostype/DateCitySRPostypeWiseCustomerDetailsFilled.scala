...
object DateCitySRPostypeWiseCustomerDetailsFilled {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCitySRPostypeWiseCustomerDetailsFilled")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCitySRPostypeWiseCustomerDetailsFilled")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("City",rdd._2).append("PosType",rdd._3).append("SalesRepNameid",rdd._4).append("SalesRepName",rdd._5).append("CustomerMobile",rdd._6).append("Count",rdd._7))
