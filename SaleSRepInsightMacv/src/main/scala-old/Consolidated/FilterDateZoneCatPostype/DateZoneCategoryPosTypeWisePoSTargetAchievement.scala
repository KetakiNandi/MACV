package FilterDateZoneCatPostype
...
object DateZoneCategoryPosTypeWisePoSTargetAchievement {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneCategoryPosTypeWisePoSTargetAchievement")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneCategoryPosTypeWisePoSTargetAchievement")
   ...
   // splitCollections.foreach(println)
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("Zone",rdd._2).append("Category",rdd._3).append("PosType",rdd._4).append("SourceSite",rdd._5).append("Actual",rdd._6))
