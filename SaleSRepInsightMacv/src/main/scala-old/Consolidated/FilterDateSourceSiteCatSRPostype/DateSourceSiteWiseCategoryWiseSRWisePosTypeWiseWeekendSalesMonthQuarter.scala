...
object DateSourceSiteWiseCategoryWiseSRWisePosTypeWiseWeekendSalesMonthQuarter {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(17)) getOrElse(""),Try(fields(5)) getOrElse("") ,Try(fields(12)) getOrElse(""),Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),Try(fields(29)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteWiseCategoryWiseSRWisePosTypeWiseWeekendSalesMonthQuarter")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteWiseCategoryWiseSRWisePosTypeWiseWeekendSalesMonthQuarter")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("Quarter",rdd._6).append("SourceSite",rdd._2).append("PosType",rdd._8).append("Category",rdd._3).append("SalesRepNameid",rdd._4).append("SalesRepName",rdd._5).append("Quantity",rdd._7))
