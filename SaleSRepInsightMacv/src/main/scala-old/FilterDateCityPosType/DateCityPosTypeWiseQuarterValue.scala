...
object DateCityPosTypeWiseQuarterValue {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(18)) getOrElse(""),Try(fields(29)) getOrElse(""),Try(fields(20)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCityPosTypeWiseQuarterValue")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCityPosTypeWiseQuarterValue")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._3).getTime)).append("City",rdd._5).append("PosType",rdd._1).append("Quarter",rdd._2).append("Amount",rdd._4))
