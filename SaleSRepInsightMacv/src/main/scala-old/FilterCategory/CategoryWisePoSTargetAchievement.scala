...
object CategoryWisePoSTargetAchievement {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.CategoryWisePoSTargetAchievement")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.CategoryWisePoSTargetAchievement")
    ...
    val Addamountpos=checkyear17.map(s=>(s._4+"@"+s._1,s._2.toInt)).reduceByKey(_+_)//posname,Addmount
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("Category",rdd._1).append("SourceSite",rdd._2).append("Actual",rdd._3))
