...
object DateSourceSiteEmpIdWiseQuarterValue {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse (""), Try(fields(18)) getOrElse (""), Try(fields(8)) getOrElse (""),Try(fields(9)) getOrElse (""),Try(fields(5)) getOrElse (""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteEmpIdWiseQuarterValue")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteEmpIdWiseQuarterValue")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._4).getTime)).append("SourceSite",rdd._5).append("SalesRepNameid",rdd._1).append("SalesRepName",rdd._2).append("Quarter",rdd._3).append("Amount",rdd._6))
