...
object DateSourceSitePosTypeWiseQuarterQuantity {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(17)) getOrElse(""),Try(fields(29)) getOrElse(""),Try(fields(5)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSitePosTypeWiseQuarterQuantity")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSitePosTypeWiseQuarterQuantity")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp", today).append("date", new Date(df.parse(rdd._3).getTime)).append("SourceSite", rdd._5).append("PosType", rdd._1).append("Quarter", rdd._2).append("Quantity", rdd._4))
