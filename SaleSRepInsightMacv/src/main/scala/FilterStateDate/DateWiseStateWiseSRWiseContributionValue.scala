...
object DateWiseStateWiseSRWiseContributionValue {
  ...
  = {
    ...
    (Try(fields(5)) getOrElse(""),Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),Try(fields(18)) getOrElse(""),
      Try(fields(20)) getOrElse(""),Try(fields(1)) getOrElse(""),Try(fields(21)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseStateWiseSRWiseContributionValue")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseStateWiseSRWiseContributionValue")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._5).getTime)).append("SourceSite",rdd._1).append("SalespersonId",rdd._2).append("Salesperson",rdd._3).append("City",rdd._4).append("Amount",rdd._7).append("State",rdd._6))
