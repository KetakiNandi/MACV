...
object PosTypeWiseZoneWiseStateWiseCity {
  ...
  = {
    ...
    (Try(fields(0)) getOrElse(""),Try(fields(22)) getOrElse(""),Try(fields(21)) getOrElse(""),Try(fields(20)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.PosTypeWiseZoneWiseStateWiseCity")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.PosTypeWiseZoneWiseStateWiseCity")
    ...
    val DistinctZoneAndPosCode = TotalFilter.map(s=>(s._1,s._2,s._3,s._4)).distinct().map(s=>(s._1,s._2+"@"+s._3+"@"+s._4))

    // DistinctZoneAndPosCode.foreach(println)

    // println("ssssssssssssssssss",disPosMaster.count())

    val JoinTwoCollections = DistinctZoneAndPosCode.join(disPosMaster).map(s=>(s._2._1,s._2._2)).map(s=>(s._1.split("@")(0),s._1.split("@")(1),s._1.split("@")(2),s._2))
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("Zone",rdd._1).append("State",rdd._2).append("City",rdd._3).append("PosType",rdd._4))
