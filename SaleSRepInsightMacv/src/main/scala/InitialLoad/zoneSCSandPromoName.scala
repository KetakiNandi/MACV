...
object zoneSCSandPromoName {
  ...
  = {
    ...
    (Try(fields(5)) getOrElse(""),Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),Try(fields(11)) getOrElse(""),Try(fields(12)) getOrElse(""),Try(fields(20)) getOrElse(""),
      Try(fields(21)) getOrElse(""),Try(fields(22)) getOrElse(""),Try(fields(10)) getOrElse (""))
  ...
  def main(args: Array[String]) {
    ...
    val DownloaddataCount = mappedSaleRepdata.map(x => (x._8,x._7,x._6,x._1,x._4,x._9.replace("-",""))).distinct().filter(s=> !s._5.isEmpty)
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("Zone",rdd._1).append("State",rdd._2).append("City",rdd._3).append("SourceSite",rdd._4).append("PromoName",rdd._5).append("PromoNo",rdd._6))
