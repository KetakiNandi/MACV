...
object DateZoneEmpIdWiseSRWiseContributionQtn {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneEmpIdWiseSRWiseContributionQtn")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneEmpIdWiseSRWiseContributionQtn")
...
//DownloaddataCount.foreach(println)
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._5).getTime)).append("SourceSite",rdd._1).append("Zone",rdd._6).append("SalespersonId",rdd._2).append("Salesperson",rdd._3).append("City",rdd._4).append("Quantity",rdd._7))
