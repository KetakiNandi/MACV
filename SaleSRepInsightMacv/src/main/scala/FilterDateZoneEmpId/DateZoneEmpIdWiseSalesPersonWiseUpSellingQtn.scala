...
object DateZoneEmpIdWiseSalesPersonWiseUpSellingQtn {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(2)) getOrElse(""),Try(fields(5)) getOrElse(""),
      Try(fields(6)) getOrElse(""),Try(fields(14)) getOrElse("") ,Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),Try(fields(22)) getOrElse(""),Try(fields(18)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneEmpIdWiseSalesPersonWiseUpSellingQtn")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneEmpIdWiseSalesPersonWiseUpSellingQtn")
...
//    //test=======
//
//    val dd =splitvalue.map(s=>(s._1.substring(3,5).toInt,s._3+"#"+s._6+"#"+s._5+"#"+s._4+"#"+s._1,s._2)).filter(s=>(s._1==1))
//
//    val reddd=dd.map(s=>(s._2,s._3.toInt)).reduceByKey(_+_).map(s=>(s._1.split("#")(0),s._1.split("#")(1),s._1.split("#")(2),s._1.split("#")(3),s._1.split("#")(4),s._2)).filter(s=>(s._4=="58"))
//
//    reddd.foreach(println)

    //end

    val finalResult=splitvalue.collect()
    ...
    finalResult.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("UniqueItemCodeCount",rdd._2).append("SourceSite",rdd._3).append("Zone",rdd._6).append("SalespersonId",rdd._4).append("Salesperson",rdd._5))
