...
object StateWiseSalesperformanceValueQuarter {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.StateWiseSalesperformanceValueQuarter")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.StateWiseSalesperformanceValueQuarter")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("State",rdd._1).append("Quarter",rdd._2).append("Amount",rdd._3))
