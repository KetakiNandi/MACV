...
object SourceSiteWiseSalesperformanceValueQuarter {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.SourceSiteWiseSalesperformanceValueQuarter")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.SourceSiteWiseSalesperformanceValueQuarter")
    ...
    val placeTotalFiltercountQuarter=TotalFiltercount.map(s=>(s._2,s._1,s._3))//City,month,Amount
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("SourceSite",rdd._1).append("Quarter",rdd._2).append("Amount",rdd._3))
