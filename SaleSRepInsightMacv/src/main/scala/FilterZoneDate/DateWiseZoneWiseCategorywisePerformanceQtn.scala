...
object DateWiseZoneWiseCategorywisePerformanceQtn {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(12)) getOrElse(""),Try(fields(17)) getOrElse(""),Try(fields(22)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseZoneWiseCategorywisePerformanceQtn")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseZoneWiseCategorywisePerformanceQtn")
   ...
   // DownloaddataCount.foreach(println)
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("Category",rdd._2).append("Zone",rdd._3).append("Quantity",rdd._4))
