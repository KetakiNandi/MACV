...
object DateStatePosTypeWisePoSTargetAchievement {
  ...
  = {
    ...
    (Try(fields(5)) getOrElse ("") ,Try(fields(18)) getOrElse (""),Try(fields(27)) getOrElse (""), Try(fields(1)) getOrElse (""),
      Try(fields(29)) getOrElse (""), Try(fields(21)) getOrElse (""))
  }
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStatePosTypeWisePoSTargetAchievement")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStatePosTypeWisePoSTargetAchievement")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp", today).append("date", new Date(df.parse(rdd._3).getTime)).append("State",rdd._4).append("PosType", rdd._1).append("SourceSite", rdd._2).append("Actual", rdd._5))
