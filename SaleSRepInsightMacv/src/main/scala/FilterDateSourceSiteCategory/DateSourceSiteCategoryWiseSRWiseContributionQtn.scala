  ...
  def main(args: Array[String]) {
   ...
   val results = finalResult.collect()
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._5).getTime)).append("SourceSite",rdd._1).append("SalespersonId",rdd._2).append("Salesperson",rdd._3).append("City",rdd._4).append("Quantity",rdd._6).append("Category",rdd._7))
