...
object DateSourceSiteCategoryWiseTopTenSellingItemsQtn {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(13)) getOrElse(""),Try(fields(14)) getOrElse(""),Try(fields(16)) getOrElse(""),
      Try(fields(5)) getOrElse(""),Try(fields(12)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteCategoryWiseTopTenSellingItemsQtn")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteCategoryWiseTopTenSellingItemsQtn")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._3).getTime)).append("Item_Code",rdd._2).append("Quantity",rdd._6).append("SourceSite",rdd._4).append("SubCategory",rdd._1).append("Category",rdd._5))
