...
object DateWiseZoneWiseWeekDaySalesMonth {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseZoneWiseWeekDaySalesMonth")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseZoneWiseWeekDaySalesMonth")
   ...
   // formattedDate.foreach(println)
    ...
    val convertMonth =dd.map(s=>(if(s._1==1){"Jan"} else if(s._1==2){"Feb"} else if(s._1==3){"Mar"} else if(s._1==4){"Apr"} else if(s._1==5){"May"} else if(s._1==6){"Jun"} else if(s._1==7){"Jul"} else if(s._1==8){"Aug"} else if(s._1==9){"Sep"} else if(s._1==10){"Oct"} else if(s._1==11){"Nov"} else {"Dec"},s._2,s._3,s._4))

    // convertMonth.foreach(println)
    val finalResult = convertMonth.map(s=>(s._1+"#"+s._2+"#"+s._4,s._3.toInt)).reduceByKey(_+_).map(s=>(s._1.split("#")(0),s._1.split("#")(1),s._1.split("#")(2),s._2))

    //test
//
//      val a = finalResult.map(s=>(s._1,s._2,s._4)).filter(s=>(s._1=="Jan"))
//
//    val b=a.map(s=>(s._2,s._3.toInt)).reduceByKey(_+_)
//
//      b.foreach(println)
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._3).getTime)).append("Month",rdd._1).append("Zone",rdd._2).append("Quantity",rdd._4))
