...
object DateSourceSiteEmpIdWiseCustomerDetailsFilled {
  ...
  = {
    ...
    (Try(fields(8)) getOrElse (""),Try(fields(9)) getOrElse (""),Try(fields(7)) getOrElse (""),Try(fields(1)) getOrElse ("") ,Try(fields(5)) getOrElse (""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteEmpIdWiseCustomerDetailsFilled")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteEmpIdWiseCustomerDetailsFilled")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("SourceSite",rdd._4).append("SalesRepNameid",rdd._2).append("SalesRepName",rdd._3).append("CustomerMobile",rdd._5).append("Count",rdd._6))
