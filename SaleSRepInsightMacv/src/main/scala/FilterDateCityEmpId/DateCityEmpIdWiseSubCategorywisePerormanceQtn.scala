...
object DateCityEmpIdWiseSubCategorywisePerormanceQtn {
  ...
  = {
    ...
    (Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),Try(fields(13)) getOrElse(""),
      Try(fields(17)) getOrElse(""),Try(fields(1)) getOrElse(""),Try(fields(20)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCityEmpIdWiseSubCategorywisePerormanceQtn")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateCityEmpIdWiseSubCategorywisePerormanceQtn")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("City",rdd._6).append("SalesRepNameid",rdd._2).append("SalesRepName",rdd._3).append("SubCategory",rdd._4).append("Quantity",rdd._5))
