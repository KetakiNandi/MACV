...
object DateWiseSRWisePosTypeWiseWeekendSalesMonth {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseSRWisePosTypeWiseWeekendSalesMonth")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateWiseSRWisePosTypeWiseWeekendSalesMonth")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("Month",rdd._6).append("PosType",rdd._2).append("SalesRepNameid",rdd._3).append("SalesRepName",rdd._4).append("Quantity",rdd._5))
