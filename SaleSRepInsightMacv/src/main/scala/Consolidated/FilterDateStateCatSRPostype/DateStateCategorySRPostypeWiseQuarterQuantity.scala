...
object DateStateCategorySRPostypeWiseQuarterQuantity {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStateCategorySRPostypeWiseQuarterQuantity")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStateCategorySRPostypeWiseQuarterQuantity")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("Quarter",rdd._8).append("SalesRepNameid",rdd._5).append("SalesRepName",rdd._6).append("State",rdd._2).append("Category",rdd._3).append("PosType",rdd._4).append("Quantity",rdd._7))
