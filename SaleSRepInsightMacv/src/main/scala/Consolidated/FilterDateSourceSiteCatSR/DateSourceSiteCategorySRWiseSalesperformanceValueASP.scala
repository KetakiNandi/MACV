...
object DateSourceSiteCategorySRWiseSalesperformanceValueASP {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteCategorySRWiseSalesperformanceValueASP")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateSourceSiteCategorySRWiseSalesperformanceValueASP")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("SourceSite",rdd._2).append("Category",rdd._3).append("SalesRepNameid",rdd._4).append("SalesRepName",rdd._5).append("Amount",rdd._6).append("CountPerday",rdd._7))
