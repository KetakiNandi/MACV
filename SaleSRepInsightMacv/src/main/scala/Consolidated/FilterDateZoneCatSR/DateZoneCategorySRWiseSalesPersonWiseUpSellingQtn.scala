...
object DateZoneCategorySRWiseSalesPersonWiseUpSellingQtn {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse(""),Try(fields(2)) getOrElse(""),Try(fields(5)) getOrElse(""),Try(fields(6)) getOrElse(""),Try(fields(14)) getOrElse("") ,Try(fields(8)) getOrElse(""),Try(fields(9)) getOrElse(""),
      Try(fields(22)) getOrElse("") ,Try(fields(12)) getOrElse(""),Try(fields(20)) getOrElse(""),Try(fields(18)) getOrElse(""))
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneCategorySRWiseSalesPersonWiseUpSellingQtn")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateZoneCategorySRWiseSalesPersonWiseUpSellingQtn")
   ...
   // b.foreach(println)
    ...
    splitvalue.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("City",rdd._8).append("UniqueItemCodeCount",rdd._2).append("SourceSite",rdd._3).append("SalespersonId",rdd._4).append("Salesperson",rdd._5).append("Zone",rdd._6).append("Category",rdd._7))

        MongoSpark.save(sc.parallelize(newDocs))

     // println(newDocs)//Date,Zone,Category,SR
