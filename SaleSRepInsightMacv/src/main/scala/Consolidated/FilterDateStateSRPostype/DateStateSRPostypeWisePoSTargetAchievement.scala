package FilterDateStateSRPostype
...
object DateStateSRPostypeWisePoSTargetAchievement {
  ...
  = {
    ...
    (Try(fields(1)) getOrElse ("") ,Try(fields(21)) getOrElse (""),Try(fields(8)) getOrElse (""), Try(fields(29)) getOrElse (""),
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStateSRPostypeWisePoSTargetAchievement")
      .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/SaleRepInsight.DateStateSRPostypeWisePoSTargetAchievement")
    ...
    results.foreach({ rdd =>
      ...
      val newDocs = Seq(new Document("TimeStamp",today).append("date",new Date(df.parse(rdd._1).getTime)).append("State",rdd._2).append("SalesRepNameid",rdd._3).append("SalesRepName",rdd._6).append("PosType",rdd._4).append("SourceSite",rdd._5).append("Actual",rdd._7))
