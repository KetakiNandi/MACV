...
if ($Sitename && $_SESSION['stDate'] && $_SESSION['endDate'] && $EmpIdname && $POSTypeparam && $categoryname){  // DONE
 ...
 $result = executeBlock($datestatecatPostypefilterSession,'DateStateCategoryPostypeWiseSalesperformanceValue');
  ...
  $result = executeBlock($datecatfilterSession,'CategoryWiseSalesPerformanceValue');
    ...
    $result = executeBlock($dateposfilterSession,'PosTypeWiseSalesPerformanceValue');
    echo $result;

}
else if ($EmpIdname && $_SESSION['stDate'] && $_SESSION['endDate'] && $POSTypeparam){ 
  
    $result = executeBlock($dateempPostypefilterSession,'DateEmpIdPostypeWiseSalesperformanceValue');
    echo $result;
...
else if($itemcodeparamfilter['Item_Code'])
{
...

    {
          ...
          $result = executeBlock($datefilter,'SalesRepWiseSalesPerformanceValue');
          echo $result;
