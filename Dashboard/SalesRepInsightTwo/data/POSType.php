...
if($zonename)
...
else if($statename)
  {
    $query = new MongoDB\Driver\Query($statefilter); 
          $rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseZoneWiseState", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["PosType"]==trim($row->PosType)){          
  ...
  else if($Cityname)
  {
    $query = new MongoDB\Driver\Query($Cityfilter); 
          $rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseZoneWiseStateWiseCity", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["PosType"]==trim($row->PosType)){          
...
else if($Sitename)
  {
    $query = new MongoDB\Driver\Query($Sitefilter); 
          ...
          $rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseZoneWiseStateWiseCityWiseSourceSite", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["PosType"]==trim($row->PosType)){          
...
else if($categoryname)
  {
    $query = new MongoDB\Driver\Query($catefilter); 
        ...
        $rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseZSCSWiseCategory", $query); //category collections
  ...
  else if($Promoname)
  {
    $query = new MongoDB\Driver\Query($promoNamefilter); 
        ...
        $rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseZSCSWisePromoName", $query); //PromoName collections
...
else if($SubCatparamfilter['SubCategory'])
  {
    $query = new MongoDB\Driver\Query($SubCatparamfilter); 
        ...
        $rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseSubCatWisePerformanceQtn", $query); //PromoName collections
