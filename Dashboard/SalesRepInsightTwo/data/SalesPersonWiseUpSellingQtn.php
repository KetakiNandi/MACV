...
if ($zonename && $_SESSION['stDate'] && $_SESSION['endDate']){
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWiseZoneWiseSalesPersonWiseUpSellingQtn", $query);
	 $result = array();
          foreach($rows as $row){
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWiseStateWiseSalesPersonWiseUpSellingQtn", $query);
	 $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWiseCityWiseSalesPersonWiseUpSellingQtn", $query);
	 $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseSalesPersonWiseUpSellingQtn", $query);
	 $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.SalesPersonWiseUpSellingQtn", $query);
	 $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWiseCategoryWiseSalesPersonWiseUpSellingQtn", $query);
	 $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.PromoNameWiseSalesPersonWiseUpSellingQtn", $query);
	 $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($zonename)
{
  ...
  $query = new MongoDB\Driver\Query($zonefilter); 
          ...
          $rows  = $connection->executeQuery("SaleRepInsight.ZoneWiseSalesPersonWiseUpSellingQtn", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($statename)
{
  ...
  $query = new MongoDB\Driver\Query($statefilter); 
          ...
          $rows  = $connection->executeQuery("SaleRepInsight.StateWiseSalesPersonWiseUpSellingQtn", $query);
          $result = array();
         foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($Cityname)
{
  ...
  $query = new MongoDB\Driver\Query($Cityfilter); 
          ...
          $rows  = $connection->executeQuery("SaleRepInsight.CityWiseSalesPersonWiseUpSellingQtn", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($categoryname)
{
  ...
  $query = new MongoDB\Driver\Query($catefilter); 
          ...
          $rows  = $connection->executeQuery("SaleRepInsight.CategoryWiseSalesPersonWiseUpSellingQtn", $query);//2nd collection
...
else if($Empfilter['SalespersonId'])
{
  ...
  $query = new MongoDB\Driver\Query($Empfilter); 
          ...
          $rows  = $connection->executeQuery("SaleRepInsight.SalesPersonWiseUpSellingQtn", $query);//2nd collection
...
else if($YesNoparamfilter['CustomerMobile'])
{
  ...
  $rows                 = $connection->executeQuery("SaleRepInsight.customerDetailsWiseSalesPersonWiseUpSellingQtn", $query);//2nd Collection saleRep,cat,pn
  $data                 = array();
  $findCounter          = 0;
  foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
...
else if($SubCatparamfilter['SubCategory'])
{
  ...
  $rows                 = $connection->executeQuery("SaleRepInsight.SubCategoryWiseSalesPersonWiseUpSellingQtn", $query);//2nd Collection saleRep,cat,pn
  $data                 = array();
  $findCounter          = 0;
  foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
...
else if($Promoname)
{
  ...
  $rows                 = $connection->executeQuery("SaleRepInsight.PromoNameWiseSalesPersonWiseUpSellingQtn", $query);//2nd Collection saleRep,cat,pn
  $data                 = array();
  $findCounter          = 0;
  foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
...
else if($itemcodeparamfilter['Item_Code'])
{
  ...
  $rows                 = $connection->executeQuery("SaleRepInsight.Top10ItemSalesWiseUpselling", $query);//2nd Collection saleRep,cat,pn
  $data                 = array();
  $findCounter          = 0;
  foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
