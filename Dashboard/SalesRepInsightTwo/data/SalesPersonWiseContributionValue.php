...
if ($zonename && $_SESSION['stDate'] && $_SESSION['endDate']){
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWiseSRWiseContributionValue", $query);
	$result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWisePosTypeWiseSRWiseContributionValue", $query);
	$result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWiseSourceSiteWiseSRWiseContributionValue", $query);
	$result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWiseCategoryWiseSRWiseContributionValue", $query);
	$result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWisePromoWiseSRWiseContributionValue", $query);
	$result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($POSTypeparam)
  {
      ...
      $rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseSRWiseContributionValue", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
echo json_encode($data);
...
else if($zonename)
  {
      ...
      $rows  = $connection->executeQuery("SaleRepInsight.ZoneWiseSRWiseContributionValue", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($statename)
  {
      ...
      $rows  = $connection->executeQuery("SaleRepInsight.StateWiseSRWiseContributionValue", $query);
          $result = array();
         foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($Cityname)
  {
      ...
      $rows  = $connection->executeQuery("SaleRepInsight.CityWiseSRWiseContributionValue", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($Sitename)
  {
      ...
      $rows  = $connection->executeQuery("SaleRepInsight.SalesPersonWiseContributionValue", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($Empfilter['SalespersonId'])
  {
      ...
      $rows  = $connection->executeQuery("SaleRepInsight.SalesPersonWiseContributionValue", $query);//2nd collection
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($categoryname)
  {
      ...
      $rows  = $connection->executeQuery("SaleRepInsight.CategoryWiseSRWiseContributionValue", $query);//2nd collection
         $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){  
...
else if($Promoname)
  {
      ...
      $rows  = $connection->executeQuery("SaleRepInsight.PromoNameWiseSRWiseContributionValue", $query);//2nd collection
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["SalespersonId"]==trim($row->SalespersonId)){ 
...
else if($YesNoparamfilter['CustomerMobile'])
{
  ...
  $rows                 = $connection->executeQuery("SaleRepInsight.CustomerDetailsWiseSRWiseContributionValue", $query);//2nd Collection saleRep,cat,pn
  $data                 = array();
  $findCounter          = 0;
  foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
...
else if($SubCatparamfilter['SubCategory'])
{
  ...
  $rows                 = $connection->executeQuery("SaleRepInsight.SubCategoryWiseSRWiseContributionValue", $query);//2nd Collection saleRep,cat,pn
  $data                 = array();
  $findCounter          = 0;
  foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
...
else if($itemcodeparamfilter['Item_Code'])
{
  ...
  $rows                 = $connection->executeQuery("SaleRepInsight.TopTensellingWiseSalespersonWiseContributionValue", $query);//2nd Collection saleRep,cat,pn
  $data                 = array();
  $findCounter          = 0;
  foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
