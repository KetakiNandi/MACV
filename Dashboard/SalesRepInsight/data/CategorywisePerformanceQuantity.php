...
if ($zonename && $_SESSION['stDate'] && $_SESSION['endDate']){
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWisePromoWiseCategorywisePerformanceQtn", $query);
	$result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["Category"]==trim($row->Category)){  
	...
	$rows  = $connection->executeQuery("SaleRepInsight.DateWiseEmpIdWiseCategorywisePerformanceQtn", $query);
	$result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["Category"]==trim($row->Category)){  
...
else if($POSTypeparam)
{
   $query = new MongoDB\Driver\Query($POSTypefilter); 
          ...
          $rows  = $connection->executeQuery("SaleRepInsight.PosTypeWiseCategoryWisePerformanceQtn", $query);
          $result = array();
          foreach($rows as $row){
              for($i=0;$i<count($data);$i++){
              if($data[$i]["Category"]==trim($row->Category)){  
